...
export default function AppHero() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setStats((prev) => {
        const newEquipments = prev.equipments >= 500 ? 500 : prev.equipments + 25;
        const newInterventions =
          prev.interventions >= 2500 ? 2500 : prev.interventions + 125;
        const newTechnicians = prev.technicians >= 50 ? 50 : prev.technicians + 2;
 
        if (
          newEquipments === 500 &&
          newInterventions === 2500 &&
          newTechnicians === 50
        ) {
      ...
      >
        <motion.div className="flex w-full flex-col items-center justify-between lg:flex-row lg:items-start">
          <div className="w-full lg:w-auto">
            ...
            >
              La Solution Complète de <br className="hidden sm:inline" />
            ...
            >
              <div className="rounded-lg border border-purple-500/20 bg-black/40 px-4 py-2 backdrop-blur-sm">
                <p className="text-2xl font-bold text-white">
                  {stats.equipments.toLocaleString()}+
                </p>
                <p className="text-xs text-gray-400">Équipements Gérés</p>
              </div>
              ...
              <div className="rounded-lg border border-indigo-500/20 bg-black/40 px-4 py-2 backdrop-blur-sm">
                ...
                <p className="text-xs text-gray-400">Techniciens</p>
              </div>
            </motion.div>
